...
export function LatestProductsSlider({ products }: { products: any[] }) {
  ...
  const latestProducts = normalizedList.filter((p) => p.isNew);
  const displayList = latestProducts.length > 0 ? latestProducts : normalizedList;
