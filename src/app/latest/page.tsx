...
import { Star, Heart, Eye, ShoppingBag, Sparkles, SlidersHorizontal, ArrowUpDown } from "lucide-react";
...
export default function LatestProductsPage() {
        ...
        if (Array.isArray(list) && list.length > 0) {
          const mapped = list.map((p: any) => {
            ...
            return {
              ...
              isNew: Boolean(p.new_arrival || p.isNew),
              discountPercentage: rawSale && rawPrice > rawSale ? Math.round(((rawPrice - rawSale) / rawPrice) * 100) : undefined,
            };
          });
          setProducts(mapped);
        } else {
  ...
  const latestProducts = sourceProducts.filter((p) => p.isNew || p.new_arrival);
  const displayItems = latestProducts.length > 0 ? latestProducts : sourceProducts;
      ...
      <section className="bg-gradient-to-r from-[#0b3b82] via-[#092a5e] to-[#b30047] text-white py-14 sm:py-20 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 text-center space-y-3 relative z-10">
          <span className="inline-flex items-center gap-2 bg-emerald-500/20 border border-emerald-400/30 text-emerald-300 font-bold text-xs px-4 py-1.5 rounded-full uppercase tracking-wider">
            <Sparkles className="w-4 h-4 text-emerald-400" /> New Arrivals &amp; Fresh Stocks
          </span>
          ...
          <p className="max-w-2xl mx-auto text-blue-100 text-xs sm:text-sm leading-relaxed">
            Explore our newest organic arrivals, fresh supplements, and recently imported health essentials.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-0 bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm divide-y sm:divide-y-0 sm:divide-x divide-slate-100">
          {filteredProducts.map((prod) => (
            ...
            >
              {/* NEW Badge */}
              <span className="absolute top-3 left-3 z-10 bg-emerald-600 text-white text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider shadow">
                NEW
              </span>
              ...
              <div>
                ...
                <div className="flex items-center gap-1 mb-2">
                  ...
                  <span className="text-[10px] font-bold text-slate-400 ml-1">({prod.reviewsCount || 8})</span>
                </div>
